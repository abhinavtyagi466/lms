...
export const UserManagement: React.FC = () => {
  ...
  const handleUpdateUser = async () => {
    try {
      ...
      const nameValidation = validateName(editUserData.name);
      if (!nameValidation.isValid) errors.name = nameValidation.error;

      const emailValidation = validateEmail(editUserData.email);
      if (!emailValidation.isValid) errors.email = emailValidation.error;

      const phoneValidation = validatePhone(editUserData.phone);
      if (!phoneValidation.isValid) errors.phone = phoneValidation.error;

      // Validate password only if it's being changed
      if (editUserData.password) {
        const passwordValidation = validatePassword(editUserData.password);
        if (!passwordValidation.isValid) errors.password = passwordValidation.error;
      }

      // Validate Aadhaar if provided
      if (editUserData.aadhaarNo) {
        const aadhaarValidation = validateAadhaar(editUserData.aadhaarNo);
      ...
      Object.keys(editUserData).forEach(key => {
        const value = editUserData[key as keyof typeof editUserData];
        // Skip documents/avatar if empty, and skip password if empty (so it doesn't reset)
        if (key !== 'documents' && key !== 'avatar' && key !== 'password') {
      ...
      // Add new avatar if selected (must be a File object)
      if (editUserData.avatar && editUserData.avatar instanceof File) {
        formData.append('avatar', editUserData.avatar);
      }

      // Add new documents if any (must be File objects)
      if (editUserData.documents && Array.isArray(editUserData.documents)) {
        editUserData.documents.forEach((doc) => {
      ...
      // Call API
      await apiService.users.updateUser(selectedUser._id, formData);
      ...
      // Notify UserDetailsPage and Dashboard to refresh
      localStorage.setItem('userUpdated', selectedUser._id);
      // Trigger custom events for same-window listeners
        ...
        case 'edit':
          ...
          if (editUser) {
            ...
            setEditUserData({
              ...
              dateOfBirth: editUser.dateOfBirth ? new Date(editUser.dateOfBirth).toISOString().split('T')[0] : '',
              fathersName: editUser.fathersName || '',
              dateOfJoining: editUser.dateOfJoining ? new Date(editUser.dateOfJoining).toISOString().split('T')[0] : '',
              reportingManager: editUser.reportingManager || '',
              highestEducation: editUser.highestEducation || '',
              currentAddress: editUser.currentAddress || '',
              location: editUser.location || '',
              city: editUser.city || '',
              state: editUser.state || '',
              region: editUser.region || '',
              aadhaarNo: editUser.aadhaarNo || '',
              panNo: editUser.panNo || '',
              documents: [],
              avatar: null
            });
          ...
          <div className="space-y-4">
            <div className="flex flex-col sm:flex-row gap-4">
              <div className="flex-1 relative">
                ...
                {searchTerm.length === 0 && (
                  <Search
                    className="w-5 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 dark:text-gray-500 pointer-events-none z-10"
                  />
                )}
                ...
                <Input
                  ...
                  className="pl-11 pr-4 py-3 w-full bg-white dark:bg-gray-700 border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white rounded-xl shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200"
                />
              </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        ...
                        <Input
                          ...
                          maxLength={12}
                          className={validationErrors.aadhaarNo ? 'border-red-500' : ''}
                        />
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          maxLength={10}
                          className={validationErrors.panNo ? 'border-red-500' : ''}
                        />
        ...
        {showEditModal && selectedUser && (
          <ModalPortal>
            <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[100] p-4">
              <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl border border-gray-100 dark:border-gray-700">
                <div className="flex items-center justify-between mb-4">
                  <h2 className="text-xl font-semibold text-gray-900 dark:text-white">Edit User Details</h2>
                  <Button
                    variant="ghost"
                    size="sm"
                    onClick={() => {
                ...
                <div className="space-y-6">
                  <div className="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800">
                    <p className="text-sm text-blue-800 dark:text-blue-200">
                      <strong>Employee ID:</strong> {selectedUser.employeeId || 'N/A'}
                    </p>
                    <p className="text-xs text-blue-600 dark:text-blue-300 mt-1">
                      Employee ID cannot be changed. Leave password empty to keep the existing password.
                    </p>
                  </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) => {
                            const value = sanitizeNumericInput(e.target.value);
                            setEditUserData(prev => ({ ...prev, phone: value }));
                            const validation = validatePhone(value);
                            setValidationErrors(prev => ({ ...prev, phone: validation.error }));
                          }}
                          placeholder="Enter 10-digit phone number"
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div>
                          <Label htmlFor="edit-fathersName">Father's Name</Label>
                          <Input
                            id="edit-fathersName"
                            value={editUserData.fathersName}
                            onChange={(e) => setEditUserData(prev => ({ ...prev, fathersName: e.target.value }))}
                            placeholder="Enter father's name"
                          />
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                          ...
                          <Input
                            ...
                            onChange={(e) => setEditUserData(prev => ({ ...prev, dateOfJoining: e.target.value }))}
                          />
                        </div>

                        <div>
                          <Label htmlFor="edit-highestEducation">Highest Education</Label>
                          <Input
                            id="edit-highestEducation"
                            value={editUserData.highestEducation}
                            onChange={(e) => setEditUserData(prev => ({ ...prev, highestEducation: e.target.value }))}
                            placeholder="Enter highest education qualification"
                          />
                        </div>
                      </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      ...
                      {/* Conditional Reporting Manager - Only for Users */}
                      {editUserData.userType === 'user' && (
                        <div>
                          <Label htmlFor="edit-reportingManager">Reporting Manager *</Label>
                          <Input
                            id="edit-reportingManager"
                            value={editUserData.reportingManager}
                            onChange={(e) => setEditUserData(prev => ({ ...prev, reportingManager: e.target.value }))}
                            placeholder="Enter reporting manager name"
                          />
                        </div>
                      )}
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        value={editUserData.currentAddress}
                        onChange={(e) => setEditUserData(prev => ({ ...prev, currentAddress: e.target.value }))}
                        placeholder="Enter complete current address"
                        rows={3}
                        className="w-full mt-1 px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
                      />
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) => setEditUserData(prev => ({ ...prev, location: e.target.value }))}
                          placeholder="Enter location/area"
                        />
                      </div>
                      <div>
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) => setEditUserData(prev => ({ ...prev, region: e.target.value }))}
                        placeholder="Enter assigned region"
                      />
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        <Label htmlFor="edit-aadhaarNo">Aadhaar Number</Label>
                        <Input
                          id="edit-aadhaarNo"
                          value={editUserData.aadhaarNo}
                          onChange={(e) => {
                            const value = sanitizeNumericInput(e.target.value);
                            setEditUserData(prev => ({ ...prev, aadhaarNo: value }));
                            if (value) {
                              const validation = validateAadhaar(value);
                              setValidationErrors(prev => ({ ...prev, aadhaarNo: validation.error }));
                            } else {
                              setValidationErrors(prev => ({ ...prev, aadhaarNo: '' }));
                            }
                          }}
                          placeholder="Enter 12-digit Aadhaar number"
                          maxLength={12}
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) => {
                            const value = sanitizePANInput(e.target.value);
                            setEditUserData(prev => ({ ...prev, panNo: value }));
                            if (value) {
                              const validation = validatePAN(value);
                              setValidationErrors(prev => ({ ...prev, panNo: validation.error }));
                            } else {
                              setValidationErrors(prev => ({ ...prev, panNo: '' }));
                            }
                          }}
                          placeholder="Enter PAN number (ABCDE1234F)"
                          maxLength={10}
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        <Label htmlFor="edit-avatar">Employee Photo</Label>
                        <Input
                          id="edit-avatar"
                          type="file"
                          accept="image/*"
                        ...
                        />
                        <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                          Upload new photo to replace existing (JPG, PNG, GIF)
                        </p>
                      </div>

                      <div>
                        <Label htmlFor="edit-documents">Documents (PDF/Images)</Label>
                        <Input
                          id="edit-documents"
                          type="file"
                          accept=".pdf,.jpg,.jpeg,.png,.gif"
                          ...
                          onChange={(e) => {
                            const files = Array.from(e.target.files || []);
                            setEditUserData(prev => ({ ...prev, documents: files }));
                          }}
                          className="file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-green-50 file:text-green-700 hover:file:bg-green-100"
                        />
                        <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                    ...
                    {editUserData.documents.length > 0 && (
                      <div className="mt-2">
                        <p className="text-sm font-medium text-gray-700 dark:text-gray-300">Selected Documents:</p>
                        <ul className="list-disc list-inside text-sm text-gray-600 dark:text-gray-400">
                          {editUserData.documents.map((doc, index) => (
                            <li key={index}>{doc.name}</li>
                          ))}
                        </ul>
                      </div>
