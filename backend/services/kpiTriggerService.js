  ...
  async processKPIFromExcel(excelData, period, submittedBy = null) {
    ...
    for (const row of excelData) {
      try {
        ...
        // Try to find existing user only (no auto-create) with robust matching
        const cleanEmployeeId = feEmployeeId ? String(feEmployeeId).trim() : '';
        const cleanEmail = feEmail ? String(feEmail).trim().toLowerCase() : '';
        const cleanName = feName ? String(feName).trim() : '';

        const orClauses = [];
        if (cleanEmployeeId) {
          const escapedId = cleanEmployeeId.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
          orClauses.push({ employeeId: { $regex: `^${escapedId}$`, $options: 'i' } });
        }
        if (cleanEmail) {
          orClauses.push({ email: cleanEmail });
        }
        if (cleanName) {
          orClauses.push({ name: { $regex: cleanName, $options: 'i' } });
        }

        const matchedUser = orClauses.length > 0
          ? await User.findOne({ $or: orClauses })
          : null;
        ...
        if (matchedUser) {
          ...
          // Unmatched: store in UnmatchedKPI
          try {
            await UnmatchedKPI.findOneAndUpdate(
              ...
              },
              {
                $set: {
                  fe: cleanName || feName || 'Unknown',
                  employeeId: cleanEmployeeId || undefined,
                  email: cleanEmail || feEmail || undefined,
                  period,
                  kpiScore,
                  rating,
                  rawData: row,
            ...
            console.error(`DB error storing unmatched user ${feName}:`, dbError.message);
          }
          ...
          if (feEmail && feEmail.trim()) {
            try {
              ...
              console.error(`Email failed for unmatched user ${feName}:`, emailError.message);
              emailsSent = [{ to: feEmail, status: 'failed', error: emailError.message }];
            }
          } else {
            console.log(`⚠️ No email for unmatched user ${feName}, skipping email`);
          }

          results.push({
            fe: feName,
            matched: false,
            employeeId: cleanEmployeeId || null,
            email: feEmail || null,
            kpiScore,
            rating,
            emailsSent,
